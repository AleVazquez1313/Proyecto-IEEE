...
$pageTitle  = 'Módulo III — Elecciones';
$pageActive = 'modulo3';

$moduloData     = datosModulo3();
$progresoModulo = 0;

if (DB_ACTIVA && $pdo) {
    try {
        $stmt = $pdo->prepare('SELECT m.*, c.nombre_curso, c.id_curso FROM aprende_modulo m JOIN aprende_curso c ON c.id_curso = m.id_curso WHERE m.id_modulo = 3');
        $stmt->execute();
        $bd = $stmt->fetch();

        if ($bd) {
            $stmtU = $pdo->prepare('SELECT * FROM aprende_unidad WHERE id_modulo = 3 ORDER BY numero_unidad');
            $stmtU->execute();
